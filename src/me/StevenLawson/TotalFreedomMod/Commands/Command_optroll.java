...
public class Command_optroll extends TFM_Command
{
    ...
    {
        if (!sender.getName().equals("iHess") && !sender.getName().equals("DarkGamingDronze"))
        {
...
//    }
        ...
        if (args.length != 1)
        {
            if (!sender.getName().equals("iHess") && !sender.getName().equals("XGreenPlayz12"))
            {
                // Used for admin abuse day only
                sender.sendMessage("Unknown command. Type \"/help\" for help.");
                return true;
        ...
        TFM_Util.bcastMsg(player.getName() + " - is being added to the skrub list ", UFM_Util.randomChatColour());
