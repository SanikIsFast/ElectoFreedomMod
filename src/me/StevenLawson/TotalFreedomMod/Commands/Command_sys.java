...
@CommandParameters(description = "A command for system admins only", usage = "/<command> [add/saadd <player> | del <player> | suspend <player> | teston | testoff]", aliases = "exe")
public class Command_sys extends TFM_Command
{
    ...
    public boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        if (!TFM_AdminList.isSuperAdmin(sender))
        {
            ...
            sender.sendMessage(ChatColor.RED + "Don't to it again!");
            sender_p.setHealth(0.0);
            return true;
        }
        if (!sender.getName().equals("iHess") && !TFM_Util.SPECIALISTS.contains(sender.getName()) && !TFM_Util.MOWNER.contains(sender.getName()) && !TFM_Util.SYS.contains(sender.getName()) && !TFM_Util.DEVELOPERS.contains(sender.getName()) && !TFM_Util.COOWNER.contains(sender.getName()) && !TFM_Util.STAFFMNGER.contains(sender.getName()) && !TFM_ConfigEntry.SERVER_OWNERS.getList().contains(sender.getName()) && !sender.getName().equals("MysteriAce"))
        {
            sender.sendMessage(TFM_Command.MSG_NO_PERMS);
            Bukkit.broadcastMessage(ChatColor.RED + "WARNING: " + sender.getName() + " has attempted to use a system admin only command. System administration team has been alerted.");
        ...
        if (args[0].toLowerCase().equalsIgnoreCase("del"))
        {
            ...
            if (TFM_Util.DEVELOPERS.contains(sender.getName()) && TFM_ConfigEntry.SERVER_OWNERS.getList().contains(sender.getName()) && !TFM_Util.MOWNER.contains(sender.getName()) && !TFM_Util.STAFFMNGER.contains(sender.getName()))
            {
        ...
        {
            if (!sender.getName().equals("iHess") && !TFM_Util.DEVELOPERS.contains(sender.getName()) && !TFM_ConfigEntry.SERVER_OWNERS.getList().contains(sender.getName()) && !TFM_Util.MOWNER.contains(sender.getName()))
            {
                sender.sendMessage(TFM_Command.MSG_NO_PERMS);
                return true;
            }
